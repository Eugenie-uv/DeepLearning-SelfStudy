...
# 两层神经网络，考虑了滑动平均模型
def inference(input, avg_class, weight1, bias1, weight2, bias2):
    # 不使用滑动平均：
    if avg_class == None:
        layer1 = tf.nn.relu(tf.matmul(input, weight1) + bias1)
        return tf.matmul(layer1, weight2)+bias2
    else:
        layer1 = tf.nn.relu(tf.matmul(input, avg_class.average(weight1)) +
...
def train(mnist):
    ...
    # 使用滑动平均后的前向传播结果
    y_average = inference(x, variable_average, w1, b1, w2, b2)
