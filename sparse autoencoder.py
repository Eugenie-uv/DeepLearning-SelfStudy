# -*- coding: utf-8 -*-
"""
Created on Thu Mar 30 15:15:35 2017
...
from keras import regularizers #约束
...
encoded = Dense(encoding_dim,  #编码：对输入进行编码
                activation='relu',
                activity_regularizer=regularizers.l1(10e-5))(input_img) #L1约束
                #正则性约束：降低模型过拟合的风险
                #注意：Keras2里，用l1代替activity_l1
                #activity_regularizer:对激活值进行约束；
                #kernel_regularizer:对核函数?进行约束；
                #bias_regularizer:对偏置进行约束
...
plot_model(autoencoder, to_file='sparse_autoencoder.png')